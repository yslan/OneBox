function [X, Quad, Qbc, Qcurve, Qfront] = extrude2d_front2box(X, Quad, Qbc, Qcurve, Qfront, Vbox, nlayers, ratio, Nlap);
...
assert(nface==4, 'front2box only accepts 4 faces');

nQ0 = size(Quad,1);
nQnew = nelf * nface * nlayers;

fprintf('ext2box: nQ= %d -> %d, nlyr= %d ratio= %g', nQ0, nQ0 + nQnew, nlayers, ratio);
if (length(ratio)==1)
   wt = zeros(nlayers,1);
   wt(1) = 1.0;
   for i=2:nlayers
   ...
   wt_acc = ratio;
   assert(nlayers==length(wt_acc), 'extrude2d_box2box: invalid custom ratio');
   fprintf(', interp c: %2.2f',wt_acc(1));
   assert(wt_acc(1) > 0, 'extrude2d_box2box: invalid custom ratio');
   for i=2:nlayers
      assert(wt_acc(i) > wt_acc(i-1), 'extrude2d_box2box: invalid custom ratio');
      fprintf(' %2.2f',wt_acc(i));
   end
   fprintf(' \n');
end
...
for f = 1:nface
   ...
   for ilyr = 1:nlayers
      ...
      Qnew(iq0+1:iq0+nelf,:) = [v1, v2, v3, v4]; % this is not water tight
