function [X,Quad,Qfront] = gen_box2d(nbx, dist, deform)
...
fpts_front(:,3) = vid(end:-1:1,1);
fpts_front(:,4) = vid(end,end:-1:1)';
fpts_front(:,1) = vid(:,end);
fpts_front(:,2) = vid(1,:)';

eids = zeros(nelf, nface);
fids = zeros(nelf, nface);
eids(:,3) = nelf:-1:1;
eids(:,4) = nelf*nelf:-nelf:nelf;
eids(:,1) = (nelf-1)*nelf+1:nelf*nelf;
eids(:,2) = 1:nelf:(nelf-1)*nelf+1;;
