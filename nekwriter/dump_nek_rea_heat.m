%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function dump_nek_rea_heat(fname,X,Hexes,CBC_list,iforder2,Curve,verbose)
...
assert(length(CBC_list)==2, 'size of CBC_list must be 2');

if size(X,2) == 2
  dump_rea_2d(fname,X,Hexes,CBC_list,iforder2,Curve,verbose);
else
  dump_rea_3d(fname,X,Hexes,CBC_list,iforder2,Curve,verbose);
end
...
function dump_rea_2d(fname,X,Hexes,CBC_list,iforder2,Curve,verbose);

   E=size(Hexes,1); 

   CBCv=CBC_list{1}; nelgv = size(CBCv,1);
   CBCt=CBC_list{2}; nelgt = size(CBCt,1);
   assert(nelgv <= nelgt, 'NEL: nelgv > nelgt!');
   assert(nelgt == E, 'Hexes and CBCt mismatched!');

   lgeomv=length(unique(abs(CBCv(:)))); 
   lgeomt=length(unique(abs(CBCt(:)))); 
   assert(lgeomv<=100,'Too many BC');
   assert(lgeomt<=100,'Too many BC');

   nface = 4;
   ctypes={'m','C','s'};
   cbcv_dmy{1}='W  ';
   for ig=2:lgeomv
     cbcv_dmy{ig}=['W' sprintf('%02d',ig-1)];
   end
   cbct_dmy{1}='t  ';
   for ig=2:lgeomt
     cbct_dmy{ig}=['t' sprintf('%02d',ig-1)];
   end
   
   fid=fopen(fname,'w');
   fprintf(fid,'%8i   %8i   %8i NEL NDIM NEL\n',nelgt,2,nelgv);
   for e=1:E;
      igroup = 0; if (e>nelgv); igroup=1; end
      fprintf(fid,'            ELEMENT%12i [    1A]    GROUP    %2d\n',e,igroup);
   ...
   o=0; nbc=zeros(1,lgeomv);nbc0=0;nbce=0;
   ...
   for e=1:nelgv
     for f=1:4
       ...
       if igeom==0
         bcf='E  '; nbc0=nbc0+1;
       elseif (igeom>0 && igeom<=lgeomv)
         bcf=cbcv_dmy{igeom}; nbc(igeom)=nbc(igeom)+1;
       else
   ...
   if (verbose>1);
      fprintf('\n   fluid: #Elements=%d  #Curves=%d  #BCs',nelgv,ncurves);
      fprintf(' %d',[nbc0,nbc,nbce]);fprintf('\n'); 
   end

   fprintf(fid,' ***** THERMAL BOUNDARY CONDITIONS ***** \n');
%  fprintf(fid,' ***** NO THERMAL BOUNDARY CONDITIONS ***** \n');
   o=0; nbc=zeros(1,lgeomt);nbc0=0;nbce=0;
   for e=1:nelgt
     for f=1:4
       cbc=CBCt(e,f);
       o1=o;o2=o;o3=o;o4=o;
       ...
       if igeom==0
         bcf='E  '; nbc0=nbc0+1;
       elseif (igeom>0 && igeom<=lgeomt)
         bcf=cbct_dmy{igeom}; nbc(igeom)=nbc(igeom)+1;
       else
         bcf='t  '; nbce=nbce+1; warning('bc id missing, put inflow %d %d',e,f);
       end
...
function dump_rea_3d(fname,X,Hexes,CBC_list,iforder2,Curve,verbose);
